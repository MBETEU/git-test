...
@section('content')
<h1 class="text-3xl text-orange-500 mb-6 text-center">Completez vos informations</h1>
<form method="POST" action="{{ route('register') }}" class="w-full max-w-sm mx-auto rounded-lg border shadow-md p-5 mb-5">
    ...
    <div class="flex justify-between items-center">
        <label for="donneur">Donneur
            <input type="radio" value="1" id="donneur" name="role_id">
            <span class="checkmark"></span>
        </label>
        <label for="receveur">Receveur
            <input type="radio" value="2" id="receveur" name="role_id">
            <span class="checkmark"></span>
        </label>
    </div>
    @error('role_id')
